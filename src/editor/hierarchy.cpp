...
void HierarchyPanel::RenderEntityNode(EntityID entity, EntityID& toDelete) {
    ...
    if (nameComponent) name = nameComponent->Value;
    ...
    if (ImGui::BeginPopupContextItem()) {
        if (ImGui::MenuItem("New")) {
            ...
            ctx.Registry.AddComponent<Components::Parent>(child,
                                                          selectedEntity);

            auto* children = ctx.Registry.TryGetComponent<Components::Children>(
                selectedEntity);
            if (!children)
                children = &ctx.Registry.AddComponent<Components::Children>(
                    selectedEntity);
...
void HierarchyPanel::OnRender() {
    ...
    m_engineContext.Registry.Each_Not<Core::ECS::Components::Parent>(
        [&](Core::ECS::EntityID id) { RenderEntityNode(id, toDelete); });
