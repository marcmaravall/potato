...
void GLFW_WindowBackend::SwapBuffers() { glfwSwapBuffers(m_glfwWindow); }

GLFW_WindowBackend::GLFW_WindowBackend(const std::string& title, size_t width,
                                       size_t height, void* share)
...
void GLFW_WindowBackend::PollEvents(Input::InputState& state) {
    ...
    for (int glfwKey : k_trackedGlfwKeys) {
        ...
        if (key == Input::Key::NONE) continue;
...
GLFW_WindowBackend::~GLFW_WindowBackend() { glfwDestroyWindow(m_glfwWindow); }
}  // namespace PotatoEngine::Core::Windowing
