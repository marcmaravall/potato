...
LuaScriptSystem::LuaScriptSystem(EngineContext& ctx) : ECS::System(ctx) {
    ...
    input.set_function("is_key", [&ctx](Input::Key key) -> bool {
        return ctx.InputState.IsKey(key);
    });
    input.set_function("is_key_down", [&ctx](Input::Key key) -> bool {
        return ctx.InputState.IsKeyDown(key);
    });
    input.set_function("is_key_up", [&ctx](Input::Key key) -> bool {
    ...
    m_lua.new_usertype<Name>(
        "Name", sol::constructors<Name(const std::string& name)>(), "value",
        &Name::Value, sol::base_classes, sol::bases<Component>());
    ...
    m_lua.new_usertype<SpriteRenderer>(
        "SpriteRenderer",
    ...
    m_lua.new_usertype<Camera>("Camera", sol::constructors<Camera()>(),
